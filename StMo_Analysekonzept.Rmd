...
plot_miv <- miv %>%
  ...
  ggplot(aes(x = as.Date(date), y = total, color = direction, group = direction)) +
  # geom_point() +
  geom_path(aes(group = direction)) +
  scale_x_date(date_labels = "%a\n%d. %m", date_breaks = "2 weeks") +
...
```
Alle sichtbaren "Einbrüche" können Feiertagen, wie Ostern, 1. Mai oder Pfingsten, zugeordnet werden.

# Modelling

**Modellselektion**

Es gibt umfassend Lektüre zu den verschiedenen Modellen zur Verkehrsprognose. Die Herausforderung besteht darin, die gewählten Methoden in der Praxis anzuwenden. 

1. Linear Regression: Modellierung basiert auf den Beobachtungen und einem Fehlerterm. Das Modellieren mit Zeitreihen bedingt, dass der Prozess stationär ist, d.h. Saisonalität muss mit modelliert werden. 

*formula: (flow ~ month + weekday + hour + holiday)*

Obwohl das Modell simpel ist, hat sich gezeigt, dass das Modell schnell recht gute Resultate liefern kann.

2. Locally Weighted Regression: Der Vorteil nicht-parametrischer Modelle ist, dass sie den dynamischen, nicht-linearen Prozess im Verkehr besser modellieren können.
